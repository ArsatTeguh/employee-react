        ...
        <div className="lg:px-6  px-2 pb-8 pt-12 relative ">
            ...
            <div className="flex flex-col  gap-1">
              ...
              <div className="flex items-center gap-6 mb-4 w-full justify-center ">
                <div className="flex items-center lg:gap-2 gap-1">
                  ...
                  <p className="text-gray-600 text-xs lg:text-sm">{task?.employee}</p>
                </div>
                <div className="flex items-center lg:gap-2 gap-1 ">
                  <p className=" text ">
                    <FaUserEdit />
                  </p>
                  <p className="text-xs lg:text-sm">{task?.action_name ?? "Not Yet"}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div
            className={`transition-all   ease-in-out   rounded-sm   ${
              message.length !== 0
                ? "h-auto mb-2  py-2 px-4  "
                : "h-0  "
            } ${message !== 'update Task' ? "bg-red-600" : " bg-green-600 "} `}
          >
            {message?.length != 0 && (
              ...
              <tr>
                <td>
                  <div className="flex items-center gap-2 cursor-pointer ">
                    ...
                    <select
                      ...
                      disabled={loading}
                      value={status}
                      className="select select-ghost !rounded border-none select-xs focus:border-none focus:ring-0 w-full lg:max-w-xs"
                    >
                ...
                <td>
                  <div className="flex items-center gap-2 cursor-pointer ">
                    ...
                    <select
                    disabled={role === roleRestrictedClient || loading}
                      value={action}
