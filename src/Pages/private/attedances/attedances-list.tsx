...
export function AttedancesList({}: Props) {
  const [searchQuery, setSearchQuery] = useState({month:GetCurrentMonthWithZero(), year:GetCurrentYear(), limit:10 });
  ...
  const handleSearch = () => {
    onFetch({
      method: "PARAMS",
      url: "/attedances",
      payload: { date: `${searchQuery.year}-${searchQuery.month}`, page, sizePage: searchQuery.limit },
    })
      .then(() => {})
  ...
  return (
    <div className='w-full p-8  lg:min-h-[745px]'>
        ...
        <p className="text-3xl font-semibold">Attedances Data</p>
