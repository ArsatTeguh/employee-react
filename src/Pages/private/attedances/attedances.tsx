...
import * as animationData from "../../../../public/indicator.json";
...
export function Attedances() {
  ...
  const {onFetch: fetchCheckout, message: msgCheckout, loading:loadCheckin,} = UseFetch()
  ...
  const handleChekin = () => {
    onFetch({method: "POST", url:"/attedance", payload: {location: "medan", project_id: project.id}}).then(() => window.location.reload())
    
  }
  const handleChekout = () => {
    onFetch({method: "PATCH", url:"/attedance"}).then(() => window.location.reload())
  }
  ...
  useEffect(() => {
    fetchCheckout({method: "POST", url:"/chekout", payload:{employee_id: user?.id }})
  },[user.id])

  return (
      <div className="w-full  lg:min-h-[745px] relative  p-8">
      {popup && (
        <Popup
          disable="employees"
        ...
        <div className="flex items-center   flex-col gap-4 justify-center !z-10 ">
           ...
           {loadCheckin ? (
             ...
             <p className="text-xl px-16 py- font-semibold py-2  border bg-primary/90 rounded text-white">
             {msgCheckout == "TRUE" ? "Punch In" : msgCheckout === "Data Not Found" ? "Punch Out" : msgCheckout}
             </p>
           )}
          ...
          <p className="font-medium">{message.length !== 0 && message}</p>
          ...
          {msgCheckout == "TRUE" && !loadCheckin  && (
             <div className="flex z-10 flex-col-reverse gap-6 border-2 lg:w-1/2 w-full p-4 border-dashed bg-zinc-100/50 rounded">
             <div className="">
               <button
                 onClick={() => setPopup(true)}
                 className="text-sm  px-8 py-2 rounded bg-primary/90 text-white hover:bg-primary font-semibold"
               >
                 Choice Project
               </button>
             </div>
             <p className=" py-2 px-8 bg-zinc-200 rounded font-semibold">
        ...
        >
         ...
         <p className="text-xl"><MdOutlineFormatListBulleted/></p>
         <p>List</p>
         </div>
        </div>
      </div>
