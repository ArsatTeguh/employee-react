...
export function ComTable({setIsAdd,setCurrentId, currentId, searchQuery,setSearchQuery, data,loading, setIsDelete, pathEdit}: Props) {
  ...
  return (
    <div className="mb-8 lg:mb-0">
          <header className=" space-y-4 p-4 sm:px-8 sm:py-6 lg:p-4 xl:px-0 xl:py-6">
    ...
    <form className="group relative">
      ...
      <input
        className="focus:ring-2 focus:ring-blue-500 lg:w-1/3 w-full focus:outline-none appearance-none text-sm leading-6 text-slate-900 placeholder-slate-400 rounded-md py-2 pl-10 ring-1 ring-slate-200 shadow-sm"
        type="text"
        ...
        <thead className="bg-primary/90 text-zinc-200">
          <tr>
            {thead.map((v, index:number) => (
                <th key={index} className="px-4 py-2 border whitespace-nowrap capitalize">{v}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {loading
            ...
            : data?.projects?.map((value: any, index: number) => (
                <tr key={index}>
                  ...
                  <td
                    className={`border whitespace-nowrap px-4 py-2  font-semibold ${
                  ...
                  <td
                    className={`text-center relative px-4 lg:px-0 ${currentId === value.id ? "bg-primary/90 text-white" : "bg-base-100 text-black"}  border `}
                  ...
                  >
                    ...
                    >
                      ...
                      <ul  tabIndex={index} className="menu border text-black dropdown-content bg-base-100 rounded  w-52 p-2 shadow">
                        <li onClick={() => router(`/${pathEdit}/${value.id}`)}>
                          <p>
